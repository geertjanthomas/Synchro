...
public partial class StaticDataDepotDeploymentDashboard : BaseDeploymentForm
{
    public StaticDataDepotDeploymentDashboard()
    {
        InitializeComponent();
        ...
        DeploymentOptionsFile = Program.Configuration["SddDeploymentOptionsFile"] ?? throw new Exception("SddDeploymentOptionsFile missing from appsettings.json");
    ...
    private void btnStart_Click(object sender, EventArgs e)
    {
        ...
        var branch = optCustomBranch.Checked ? CustomBranch.Text : "main";
        RunPowershellScript("SddPowershellScript", $"-branchName {branch}");
    }
    ...
    protected override void ReadDeploymentOptions<T>(string filename)
    {
        ...
        var branchname = RawOptions.DeploymentOptions?["BranchName"].ToString();
        if (branchname != "main")
        { 
            CustomBranch.Text = branchname; 
            optCustomBranch.Checked = true;
        }
    }

    protected override void AddCustomOptions(Dictionary<string, object> options)
    {
        var branchName = Controls.Find("CustomBranch", true).FirstOrDefault() as TextBox;
        ...
        options.Add("BranchName", optCustomBranch.Checked ? CustomBranch.Text : "main");
    }
    ...
    private void Control_UpdateExplanation(object? sender, EventArgs e)
    {
        ...
        else if (sender == chkDatabase)
            text = "Deploy the StaticDataDepot database";
        else if (sender == optMain)
